...
/*   Updated: 2021/11/21 16:25:50 by ncallie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_game	*ft_init_malloc(void)
{
	...
	game->var = (t_var *)malloc(sizeof(t_var));
	...
	game->coin = (t_img *)malloc(sizeof(t_img) * 5);
	game->player->player = (t_img *)malloc(sizeof(t_img));
	game->var->var = (t_img *)malloc(sizeof(t_img));
	if (!(game || !game->map) || !(game->mlx) || !(game->wall) || !(game->grass)
		|| !(game->exit_0) || !(game->player->player)
		|| !(game->exit_1))
...
{
	game->var->x = game->var->start_x;
	game->var->y = game->var->start_y;
}
...
void	ft_init_img(t_game *game)
{
	...
	file_init(game, &game->coin[0], COIN);
	file_init(game, &game->coin[1], COIN1);
	file_init(game, &game->coin[2], COIN2);
	file_init(game, &game->coin[3], COIN3);
	file_init(game, &game->coin[4], COIN4);
	file_init(game, game->exit_0, EXIT_0);
	file_init(game, game->exit_1, EXIT_1);
	file_init(game, game->player->player, PLAYER);
	file_init(game, game->var->var, VAR);
	ft_render(game, 5);
}
